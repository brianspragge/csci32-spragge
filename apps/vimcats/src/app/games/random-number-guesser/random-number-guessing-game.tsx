...
export default function RandomNumberGuessingGame({
  randomNumber,
  endGame,
  maxGuessCount,
}: RandomNumberGuessingGameEngineProps) {
  ...
  return (
    <div
      className={`${maxGuessCount - 1 === guessCount ? 'bg-red-50' : ''} ${maxGuessCount == guessCount ? 'bg-red-200' : ''} ${hasWon ? '!bg-green-100' : ''} p-10 rounded-md transition-colors`}
    >
      {hasWon ? (
